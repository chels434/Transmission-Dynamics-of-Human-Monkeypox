...
The crude absolute secondary attack rate, $AR_{crude}$, was found by dividing $n_{inf}$, the number of contacts subsequently infected by $n_{total}$, the total number of contacts reported \ref{eqn:equation 1}. To determine the $R_{eff}$, we multiplied $AR_{crude}$ by $\bar{n}$, the average number of contacts per case (Equation 2).3  All analysis were carried out in Excel and R.10
